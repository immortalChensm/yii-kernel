...
class CUrlManager extends CApplicationComponent
{
	...
	/**
     * 返回请求的uri[path]信息或是返回正则解析好的路由地址uri
	...
	public function parseUrl($request)
	{
		if($this->getUrlFormat()===self::PATH_FORMAT)
		{
			...
			foreach($this->_rules as $i=>$rule)
			{
			    //从路由正规映射池里取出CUrlRule对象
				if(is_array($rule))
					$this->_rules[$i]=$rule=Yii::createComponent($rule);
				if(($r=$rule->parseUrl($this,$request,$pathInfo,$rawPathInfo))!==false)
				    //如果路由含有r=xxx/xxx形式则返回，否则返回解析好的controller/action字符
...
class CUrlRule extends CBaseUrlRule
{
	...
	public function __construct($route,$pattern)
	{
		...
		if(strpos($route,'<')!==false && preg_match_all('/<(\w+)>/',$route,$matches2))
		{
			foreach($matches2[1] as $name)
			    //$this->references[controller]="<controller>";
		...
		if($this->references!==array())
		    //echo "routePattern=".$this->routePattern;
		    //routePattern=/^(?P\w+)\/(?P\w+)$/u 保存路由的正则表达式
			$this->routePattern='/^'.strtr($this->route,$tr2).'$/u';
            //echo "routePattern=".$this->routePattern."<br />";
	...
	public function parseUrl($manager,$request,$pathInfo,$rawPathInfo)
	{
		...
		if(preg_match($this->pattern.$case,$pathInfo,$matches))
		{
            ...
            //print_r($tr);
            //echo strtr($this->route,$tr);
            //print_r($this->route);
            //print_r($_REQUEST);
			if($pathInfo!==$matches[0]) // there're additional GET params
				$manager->parsePathInfo(ltrim(substr($pathInfo,strlen($matches[0])),'/'));
			if($this->routePattern!==null)
			    //将uri路由信息转换为controller/action返回
