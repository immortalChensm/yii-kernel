...
class CHttpRequest extends CApplicationComponent
{
	...
	protected function normalizeRequest()
	{
		...
		if($this->enableCsrfValidation)
		    //添加的事件池
		    //CComponent->>_e[onBeginRequest] = CList 该对象实现了基本的数组迭代，数组接口，统计等接口
		    //Clist->_d[] = [CHttpRequest,'validateCsrfToken']
     ...
     *
     * 功能主要是验证用户请求的cookie是否包含有指定的掩码token防止跨站请求
	 */
	public function validateCsrfToken($event)
	{
		...
		{
		    //判断请求方式
            //将Cookie数组元素映射为CHttpCookie组装为数组返回
...
class CCookieCollection extends CMap
{
     ...
     * 将cookie全局数组循环并映射为CHttpCookie保存返回
	 */
	protected function getCookies()
	{
		...
		{
		    //从yii/base/SecurityManager 找到这个类的实例
