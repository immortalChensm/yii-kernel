...
abstract class CApplication extends CModule
{
	...
	{
	    //实例CWebApplication 时
        //将CwebApplication对象以静态变量形式保存在YiiBase里
        //所以Yii::app() = new CWebApplication
		Yii::setApplication($this);

		// set basePath as early as possible to avoid trouble
		if(is_string($config))
		    //引入框架的配置文件主要是config/main.php 由入口决定
			$config=require($config);
		if(isset($config['basePath']))
		{
		    //将配置文件的basePath 保存在$this->_basePath
			$this->setBasePath($config['basePath']);
			unset($config['basePath']);
		}
		else
			$this->setBasePath('protected');//或是保存protected目录

        //将basePath目录设置别名保存在YiiBase
        //$_aliases[application] = path
		...
		//设置时将会触发拦截器_set从而执行CModule里的setxxx方法
        //print_r($config);
        //print_r(Yii::imports);
		...
		//实例CHttpRequest 激活onBeginRequest事件并投入到事件池里
        //事件池结构
        //[事件名称】=CList对象
        //Clist->_d[] = 对应的对象如CHTTPRequest
		$this->init();
	}
	...
	public function run()
	{
	    //是否存在指定的事件【从事件池里取】框架实例化CHttpRequest的时候存在了
		if($this->hasEventHandler('onBeginRequest'))
		    //激活事件池里的指定事件
		    //此时会运行CHttprequest 的validateCsrfToken方法
		    //运行该方法，会验证指定的请求cookie是否包含有指定的掩码防止跨站请求
		    //只是验证post,put,delete,patch请求
