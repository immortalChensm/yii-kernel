...
class CComponent
{
	 ...
	 * @param CEvent $event the event parameter 事件参数[可能是个Cevent对象]
	 * @throws CException if the event is undefined or an event handler is invalid.
     * 激活事件池里的方法
     * 就看各组件添加往事件池里添加事件时，指定了什么玩意如CHttpRequest
	 */
		...
		if(isset($this->_e[$name]))
		{
			foreach($this->_e[$name] as $handler)
			{
				...
				elseif(is_callable($handler,true))
				{
				    //从事件池里取出的元素都是CList对象
                    //但是这个对象是可以迭代的，因为它继承了
                    //ArrayAccess,Iterator,Count这种接口
					if(is_array($handler))
					{
						// an array: 0 - object, 1 - method name
                        //得到池里的对象，方法
                        //如Yii::app()->attachEventHandler('onBeginRequest',array('CHttpRequest,'validateCsrfToken'));
                        //会到得CHttpRequest 对象 validateCsrfToken方法
